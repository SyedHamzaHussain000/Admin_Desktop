    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <p style={{ fontSize: 30, fontWeight: 'bold' }}>Aborted</p>
...
>
  {isloading ? (
    ...
    <>
      {reports?.some((e) => e.status === "Aborted") ? ( // Check if any report has status "Aborted"
        reports?.map((e, i) => {
          if (e.status === "Aborted") {
            return (
              <div
          ...
          return null; // Skip rendering if status is not "Aborted"
        })
      ) : (
        <div
          style={{
